...
public class TosContainerDaoImpl implements TostContainerDao {
    ...
    private TypedQuery<TosContainerStack> querySelectAllTasksFromUser(String orderNo) {
        return em.createQuery("SELECT t FROM TOS_CONTAINER_LAST_STACK t WHERE t.ORDER_NO = ?1", TosContainerStack.class).setParameter(1, orderNo);
    }
	...
	public List<TosContainerStack> getAll() {
        return em.createQuery("SELECT t FROM TOS_CONTAINER_LAST_STACK t", TosContainerStack.class)
            .getResultList();
	}

	@Override
	public List<TosContainerStack> getRange(int offset, int count) {
        TypedQuery<TosContainerStack> query =  em.createQuery("SELECT t FROM TOS_CONTAINER_LAST_STACK t", TosContainerStack.class);
